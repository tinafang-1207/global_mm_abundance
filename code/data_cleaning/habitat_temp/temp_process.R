...
sst_rast <- readRDS("data/habitat_gis/sst_rast.rds")
usa <- rnaturalearth::ne_states(country = "United States of America", returnclass = "sf")
mexico <- rnaturalearth::ne_countries(country = "Mexico", returnclass = "sf")
states <- rnaturalearth::ne_states(country = "United States of America", returnclass = "sf") %>%
  st_transform(4326)
usa_coastline <- read_sf("data/habitat_gis/tl_2025_us_coastline/tl_2025_us_coastline.shp")
...
saveRDS(species_habitat, "data/habitat_gis/species_habitat_final.rds")
...
ggplot() +
  ...
  geom_sf(data = mexico, fill = "grey85", col = "white",
          linewidth = 0.01, inherit.aes = FALSE) +
  geom_sf(data = oregon_buffer_pacific,
