...
data <- catch_orig %>%
  ...
  left_join(area, by = c("ocean", "area")) %>%
  select(year, ocean, area, min_lat, max_lat, min_long, max_long, start_date, end_date, everything()) %>%
  # clean category
...
data_clean <- data %>%
  filter(ocean == "North Pacific" & nation %in% c("USSR", "USA", "Japan"))
